...
def plot_and_save_to_pdf(group_name, time_values, integrated_data, integratedUnit, interval_seconds=specified_interval, convergence_tolerance=tol):
    ...
    # Save plot to PDF
    pdf_filename = f"{group_name}_integrated_data_plots.pdf"
    try:
        with PdfPages(pdf_filename) as pdf:
            ...
            plt.title(f'Integrated Data for {group_name}')
            ...
            for component_values, component_label, color in zip([interval_values_x, interval_values_y, interval_values_z],
                ...
                max_value = max(max(component_values))
                buffer = max_value * 0.3
                ax.set_ylim(0, max_value + buffer)
                ...
                for i, (interval_start, interval_end) in enumerate(intervals):
                    ...
                    if not np.isnan(prev_vals).any() and np.isfinite(prev_vals).all():
                        ...
                        y_pos = max_value + buffer * 0.25 if i % 2 == 0 else max_value + buffer * 0.35
...
for group_name, group_files in integrated_groups.items():
    time_values, integrated_data = aggregate_data(group_files, is_vector=True)
    # Plot and save data to PDF
    plot_and_save_to_pdf(group_name, time_values, integrated_data, integratedUnit)